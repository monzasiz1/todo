...
module.exports = async function handler(req, res) {
  ...
  if (segments.length === 0 && req.method === 'POST') {
    try {
      ...
      const result = await pool.query(
        `INSERT INTO groups (name, description, color, icon, image_url, invite_code, created_by, is_public)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
        [name.trim(), description || '', color || '#007AFF', icon || 'users', image_url || null, inviteCode, user.id, is_public === true]
      );
  ...
  // ============================================
  // GET /api/groups/search?q=... — Search public groups
  // ============================================
  if (segments[0] === 'search' && segments.length === 1 && req.method === 'GET') {
    try {
      ...
      const result = await pool.query(
        `SELECT g.id, g.name, g.description, g.color, g.image_url,
                (SELECT COUNT(*) FROM group_members WHERE group_id = g.id) as member_count,
                (SELECT status FROM group_join_requests WHERE group_id = g.id AND user_id = $2 LIMIT 1) as my_request_status,
                EXISTS(SELECT 1 FROM group_members WHERE group_id = g.id AND user_id = $2) as already_member
         FROM groups g
         WHERE g.is_public = true
           AND (
             g.name ILIKE $1
             OR COALESCE(g.description, '') ILIKE $1
             OR replace(replace(replace(replace(lower(g.name), 'ä', 'ae'), 'ö', 'oe'), 'ü', 'ue'), 'ß', 'ss') LIKE $3
             OR replace(replace(replace(replace(lower(COALESCE(g.description, '')), 'ä', 'ae'), 'ö', 'oe'), 'ü', 'ue'), 'ß', 'ss') LIKE $3
           )
         ORDER BY g.name
         LIMIT 20`,
        [`%${q}%`, user.id, `%${normalizedQ}%`]
      );
      return res.json({ groups: result.rows });
    } catch (err) {
      console.error('Search groups error:', err);
      return res.status(500).json({ error: 'Fehler bei der Gruppensuche' });
    }
  }

  // ============================================
  // GET /api/groups/my-requests — User's own join requests
  // ============================================
    ...
    try {
      ...
      const tasksResult = await pool.query(
        `SELECT t.*, c.name as category_name, c.color as category_color,
           ...
           CASE
             WHEN t.visibility = 'selected_users' AND (t.user_id = $3 OR EXISTS (
               SELECT 1 FROM task_permissions tp_self
               WHERE tp_self.task_id = t.id AND tp_self.user_id = $3 AND tp_self.can_view = true
             )) THEN COALESCE((
             ...
             WHEN t.visibility = 'selected_users' THEN '[]'::json
             ELSE COALESCE((
               SELECT json_agg(json_build_object('user_id', u3.id, 'name', u3.name, 'avatar_color', u3.avatar_color, 'avatar_url', u3.avatar_url))
               FROM task_permissions tp
               JOIN users u3 ON u3.id = tp.user_id
  ...
  if (segments.length === 1 && req.method === 'PUT') {
    try {
      ...
      const { name, description, color, icon, image_url, is_public } = req.body;
      const result = await pool.query(
        `UPDATE groups SET name = COALESCE($1, name), description = COALESCE($2, description),
         ...
         is_public = COALESCE($7, is_public), updated_at = NOW()
         WHERE id = $6 RETURNING *`,
        [name, description, color, icon, image_url || null, groupId, is_public !== undefined ? is_public : null]
      );
  ...
  // ============================================
  // GET /api/groups/:id/dismissed-tasks — Eigene aus dem Kalender entfernte Tasks dieser Gruppe
  // ============================================
  if (segments.length === 2 && segments[1] === 'dismissed-tasks' && req.method === 'GET') {
    try {
      const groupId = segments[0];
      const membership = await getMembership(groupId);
      if (!membership) return res.status(403).json({ error: 'Kein Zugriff auf diese Gruppe' });

      // Existiert task_dismissals?
      const tableCheck = await pool.query(
        `SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_dismissals') as has_dismissals`
      );
      if (!tableCheck.rows[0]?.has_dismissals) {
      ...
      const result = await pool.query(
        `SELECT t.*, c.name as category_name, c.color as category_color,
           ...
           td.dismissed_at,
           $2::text as my_group_role
         FROM task_dismissals td
         JOIN tasks t ON t.id = td.task_id
         JOIN group_tasks gt ON gt.task_id = t.id AND gt.group_id = $1
         LEFT JOIN categories c ON t.category_id = c.id
         LEFT JOIN group_categories gc ON gc.id = gt.group_category_id
         LEFT JOIN users u ON gt.created_by = u.id
         LEFT JOIN group_subgroups gs ON gs.id = gt.subgroup_id
         ...
         ORDER BY td.dismissed_at DESC`,
        [groupId, membership.role, user.id]
      );

      return res.json({ tasks: result.rows });
    } catch (err) {
      console.error('List dismissed tasks error:', err);
      return res.status(500).json({ error: 'Fehler beim Laden entfernter Aufgaben' });
    }
  }
  ...
  if (segments.length === 3 && segments[1] === 'members' && req.method === 'DELETE') {
    try {
      ...
      // Admins can remove all non-owner members (same as owner except delete-group).
      if (membership.role === 'admin' && !isSelf && target.rows[0].role === 'owner') {
  ...
  // ============================================
  // GET /api/groups/:id/search-users?q=... — Admin: find users not in group
  // ============================================
  if (segments.length === 2 && segments[1] === 'search-users' && req.method === 'GET') {
    try {
      const groupId = segments[0];
      const membership = await getMembership(groupId);
      if (!membership || membership.role === 'member') {
        return res.status(403).json({ error: 'Nur Admins können Nutzer suchen' });
      }
      const q = String(req.query?.q || '').trim();
      ...
      return res.status(500).json({ error: 'Fehler bei der Nutzersuche' });
    }
  }
  ...
  // ============================================
  // POST /api/groups/:id/join-request — User: send join request
  // ============================================
  if (segments.length === 2 && segments[1] === 'join-request' && req.method === 'POST') {
    try {
      const groupId = segments[0];

      // Group must be public OR have join requests enabled
      const grpCheck = await pool.query(
        'SELECT id, name, is_public FROM groups WHERE id = $1',
        [groupId]
      );
      if (!grpCheck.rows[0]) return res.status(404).json({ error: 'Gruppe nicht gefunden' });
      if (!grpCheck.rows[0].is_public) {
        return res.status(403).json({ error: 'Diese Gruppe nimmt keine Anfragen entgegen' });
      }
      ...
      await pool.query(
        `INSERT INTO group_join_requests (group_id, user_id, status, message)
         VALUES ($1, $2, 'pending', $3)
         ON CONFLICT (group_id, user_id)
         DO UPDATE SET status = 'pending', message = EXCLUDED.message, updated_at = NOW()`,
        [groupId, user.id, String(message).substring(0, 300)]
      );

      // Notify admins/owner
      const admins = await pool.query(
        `SELECT user_id FROM group_members WHERE group_id = $1 AND role IN ('owner','admin')`,
        [groupId]
      );
      const senderInfo = await pool.query('SELECT name FROM users WHERE id = $1', [user.id]);
      const senderName = senderInfo.rows[0]?.name || 'Jemand';
      for (const admin of admins.rows) {
        await sendPushToUser(admin.user_id, {
          title: 'Neue Beitrittsanfrage',
          body: `${senderName} möchte der Gruppe "${grpCheck.rows[0].name}" beitreten.`,
          tag: `join-req-${groupId}-${user.id}`,
          url: `/groups?group=${groupId}`,
        }, 'group_join_request', null, groupId).catch(() => null);
      }
  ...
  // ============================================
  if (segments.length === 2 && segments[1] === 'join-requests' && req.method === 'GET') {
    try {
      const groupId = segments[0];
      const membership = await getMembership(groupId);
      if (!membership || membership.role === 'member') {
        return res.status(403).json({ error: 'Nur Admins können Anfragen sehen' });
      }
      const result = await pool.query(
        `SELECT gjr.id, gjr.user_id, gjr.status, gjr.message, gjr.created_at,
                u.name as user_name, u.avatar_color as user_color, u.avatar_url as user_avatar_url
         FROM group_join_requests gjr
  ...
  // ============================================
  // PATCH /api/groups/:id/join-requests/:requestId — Admin: accept or reject
  // ============================================
  if (segments.length === 3 && segments[1] === 'join-requests' && req.method === 'PATCH') {
    try {
      const groupId = segments[0];
      const requestId = segments[2];
      const membership = await getMembership(groupId);
      if (!membership || membership.role === 'member') {
        return res.status(403).json({ error: 'Nur Admins können Anfragen bearbeiten' });
      }
      ...
      const reqRow = await pool.query(
        `SELECT gjr.*, u.name as user_name
         FROM group_join_requests gjr JOIN users u ON u.id = gjr.user_id
         WHERE gjr.id = $1 AND gjr.group_id = $2`,
        [requestId, groupId]
      );
      if (!reqRow.rows[0]) return res.status(404).json({ error: 'Anfrage nicht gefunden' });
      const req_ = reqRow.rows[0];

      const newStatus = action === 'accept' ? 'accepted' : 'rejected';
      await pool.query(
        `UPDATE group_join_requests SET status = $1, updated_at = NOW() WHERE id = $2`,
        [newStatus, requestId]
      );
      ...
      if (action === 'accept') {
        await pool.query(
          `INSERT INTO group_members (group_id, user_id, role) VALUES ($1, $2, 'member')
           ON CONFLICT (group_id, user_id) DO NOTHING`,
          [groupId, req_.user_id]
        );
        const grpInfo = await pool.query('SELECT name FROM groups WHERE id = $1', [groupId]);
        await sendPushToUser(req_.user_id, {
          title: 'Beitrittsanfrage angenommen',
          body: `Du bist jetzt Mitglied der Gruppe "${grpInfo.rows[0]?.name || ''}".`,
          tag: `join-accepted-${groupId}`,
          url: `/groups?group=${groupId}`,
        }, 'group_join_accepted', null, groupId).catch(() => null);
      } else {
        const grpInfo = await pool.query('SELECT name FROM groups WHERE id = $1', [groupId]);
        await sendPushToUser(req_.user_id, {
          title: 'Beitrittsanfrage abgelehnt',
          body: `Deine Anfrage für die Gruppe "${grpInfo.rows[0]?.name || ''}" wurde abgelehnt.`,
          tag: `join-rejected-${groupId}`,
          url: '/groups',
        }, 'group_join_rejected', null, groupId).catch(() => null);
      }

      return res.json({ success: true, action });
    } catch (err) {
      console.error('Handle join request error:', err);
      return res.status(500).json({ error: 'Fehler beim Bearbeiten der Anfrage' });
    }
  }
