      ...
      <tr>
        <td bgcolor="#007AFF" style="background-color:#007AFF;border-radius:16px 16px 0 0;padding:28px 48px;">
          <table role="presentation" width="100%"><tr>
            ...
            <td align="right" style="vertical-align:middle;">
              <span style="font-size:12px;font-weight:600;color:rgba(255,255,255,0.6);font-family:Inter,-apple-system,Arial,sans-serif;letter-spacing:0.05em;text-transform:uppercase;">BeeQu</span>
            </td>
          </tr></table>
        </td>
      </tr>
...
<table width="560" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;width:100%;">
  <tr><td bgcolor="#007AFF" style="background:#007AFF;border-radius:16px 16px 0 0;padding:28px 48px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
      ...
      <td align="right" style="font-size:12px;font-weight:600;color:rgba(255,255,255,0.6);text-transform:uppercase;letter-spacing:0.05em;">BeeQu</td>
    </tr></table>
  </td></tr>
...
async function sendVerificationCodeMail({ to, name, code }) {
  const t = getTransporter();
  if (!t) {
    console.warn('Kein SMTP-Transporter – Mail übersprungen.');
    console.log('Verifikationscode für', to, ':', code);
    return;
  }
  const firstName = name.split(' ')[0];
  await t.sendMail({
    from:    process.env.SMTP_FROM || process.env.SMTP_USER,
    to,
    subject: `${code} – dein BeeQu Verifizierungscode`,
    html: `<!DOCTYPE html>
<html lang="de">
<head>
<meta charset="UTF-8">
...
@media only screen and (max-width:600px){.w-full{width:100%!important}.px-m{padding-left:28px!important;padding-right:28px!important}}
</style>
</head>
<body>
...
<table role="presentation" class="w-full" width="560" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;width:100%;">
  ...
  <tr><td bgcolor="#007AFF" style="background:#007AFF;border-radius:16px 16px 0 0;padding:28px 48px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
      ...
      <td align="right"><p style="margin:0;font-size:12px;color:rgba(255,255,255,0.7);font-weight:500;letter-spacing:0.5px;text-transform:uppercase;">Konto bestätigen</p></td>
    </tr></table>
  </td></tr>
  <!-- BODY -->
  <tr><td bgcolor="#FFFFFF" style="background:#fff;padding:40px 48px 32px;" class="px-m">
    <p style="margin:0 0 8px;font-size:13px;font-weight:600;color:#007AFF;text-transform:uppercase;letter-spacing:0.8px;">Hallo ${firstName},</p>
    <h1 style="margin:0 0 16px;font-size:26px;font-weight:700;color:#111827;line-height:1.2;">Dein Verifizierungscode</h1>
    <p style="margin:0 0 32px;font-size:15px;color:#4B5563;line-height:1.6;">
      Gib diesen 6-stelligen Code in der App ein, um dein BeeQu-Konto zu aktivieren. Der Code ist <strong>10 Minuten</strong> gültig.
    </p>
    <!-- CODE BOX -->
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:32px;">
      <tr><td align="center">
        <div style="display:inline-block;background:#F0F7FF;border:2px solid #007AFF;border-radius:16px;padding:24px 48px;">
          <p style="margin:0;font-size:42px;font-weight:800;color:#007AFF;letter-spacing:12px;font-variant-numeric:tabular-nums;">${code}</p>
        </div>
      </td></tr>
    </table>
    <p style="margin:0 0 8px;font-size:13px;color:#9CA3AF;text-align:center;">
      Falls du kein Konto bei BeeQu erstellt hast, kannst du diese E-Mail ignorieren.
    </p>
  </td></tr>
  <!-- FOOTER -->
  <tr><td bgcolor="#F9FAFB" style="background:#F9FAFB;border-radius:0 0 16px 16px;padding:20px 48px;border-top:1px solid #E5E7EB;">
...
module.exports = { sendActivationMail, sendVerificationCodeMail, sendPasswordChangedMail, sendPasswordChangeConfirmMail, sendPasswordResetMail };
