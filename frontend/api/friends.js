  ...
  if (segments[0] === 'invite' && req.method === 'POST') {
    try {
      ...
      if (existing.rows.length > 0) {
        ...
        // Bei abgelehnter Anfrage: alten Eintrag entfernen damit eine neue Anfrage möglich ist
        if (s === 'rejected' || s === 'declined') {
          await pool.query('DELETE FROM friends WHERE id = $1', [existing.rows[0].id]);
        }
      }
      ...
      const result = await pool.query(
        `INSERT INTO friends (user_id, friend_id, status, invite_code)
         ...
         ON CONFLICT (user_id, friend_id) DO UPDATE
           SET status = 'pending', invite_code = EXCLUDED.invite_code, created_at = NOW()
         RETURNING *`,
