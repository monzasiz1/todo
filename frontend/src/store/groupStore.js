...
function getGroupsCacheKey() {
  try {
    const token = localStorage.getItem('token') || 'anon';
    return `${GROUP_CACHE_KEY}:${token.slice(0, 24)}`;
  } catch {
    return `${GROUP_CACHE_KEY}:anon`;
  }
}

function readGroupCache() {
  try {
    const raw = localStorage.getItem(getGroupsCacheKey());
...
function writeGroupCache(state) {
  try {
    localStorage.setItem(getGroupsCacheKey(), JSON.stringify({
      groups: state.groups || [],
...
export const useGroupStore = create((set, get) => ({
  ...
  removeTaskLocally: (taskId) => {
    set((s) => {
      ...
      const nextTasks = s.groupTasks.filter((t) => String(t.id) !== key);
      if (nextTasks.length === s.groupTasks.length) return s;
