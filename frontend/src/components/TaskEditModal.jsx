...
export default function TaskEditModal({ task, onClose, onSaved }) {
  ...
  const ownerUserId = Number(task?.user_id);
  const currentUserId = (() => {
    try {
  ...
  const isBlockedShareTarget = (rawId) => {
    const id = Number(rawId);
    if (!Number.isFinite(id)) return true;
    if (Number.isFinite(ownerUserId) && id === ownerUserId) return true;
    if (Number.isFinite(currentUserId) && id === currentUserId) return true;
    return false;
  };
      ...
      if (seriesTaskId) {
        try {
          ...
          setPermissions(
            ...
            })).filter((p) => !isBlockedShareTarget(p.user_id))
          );
        } catch {
                        ...
                        <div className="task-edit-add-friends">
                          ...
                          <div className="task-edit-add-friends-avatars" role="list" aria-label="Freunde hinzufügen">
                            {friends
                              ...
                              .filter(f => !isBlockedShareTarget(f.friend_user_id))
                              .map((friend) => (
                          ...
                          {friends
                            .filter(f => !permissions.find(p => p.user_id === f.friend_user_id))
                            .filter(f => !isBlockedShareTarget(f.friend_user_id)).length === 0 && (
