...
// DSGVO-Fix: User-spezifische Calendar Settings
...
function getUserCalendarSetting(key, defaultValue = '') {
  try {
    return localStorage.getItem(getUserSpecificKey(key)) || defaultValue;
  } catch {
    return defaultValue;
  }
}

function setUserCalendarSetting(key, value) {
  try {
...
const getTaskSource = (t) => {
  const isOwner = t.is_owner === true;
  const isGroupMember = t.is_group_member === true || (t.is_group_member === undefined && t.my_group_role != null);
  if ((t.group_id || t.group_name) && (isOwner || isGroupMember)) {
    return {
...
export default function Calendar({ onDayClick, tasks: tasksProp, onVisibleRangeChange, onTaskUpdated, onTaskCreated }) {
  ...
  const [holidayColor, setHolidayColorState] = useState(() => {
    return getUserCalendarSetting(CALENDAR_HOLIDAY_COLOR_KEY, DEFAULT_HOLIDAY_COLOR);
  });
  ...
  const setHolidayColor = useCallback((color) => {
    ...
    try { setUserCalendarSetting(CALENDAR_HOLIDAY_COLOR_KEY, color); } catch { /* ignore */ }
  }, []);
  ...
  const [groupsShownInOwn, setGroupsShownInOwn] = useState(() => {
    try {
      const raw = getUserCalendarSetting(CALENDAR_GROUPS_IN_OWN_KEY, '{}');
  ...
  useEffect(() => {
    setUserCalendarSetting(CALENDAR_HOLIDAY_STATE_KEY, holidayStateCode);
  }, [holidayStateCode]);
    ...
    api.getProfile()
      .then((res) => {
        ...
        if (/^#[0-9A-Fa-f]{6}$/.test(remoteColor)) {
          ...
          setUserCalendarSetting(CALENDAR_HOLIDAY_COLOR_KEY, remoteColor);
          return;
        }
  ...
  useEffect(() => {
    setUserCalendarSetting(CALENDAR_ACTIVE_SOURCE_KEY, activeCalendarSource);
  }, [activeCalendarSource]);

  useEffect(() => {
    setUserCalendarSetting(CALENDAR_GROUPS_IN_OWN_KEY, JSON.stringify(groupsShownInOwn));
  }, [groupsShownInOwn]);
  ...
  const monthViewData = useMemo(() => {
    ...
    days.forEach((d, dayGlobalIdx) => {
      ...
      const allDayTasks = (calendarEntriesByVisibleDate.get(dayKey) || [])
        .map((t, originalIdx) => ({
          t,
          originalIdx,
      ...
      const decoratedDayTasks = [];

      // First pass: assign lanes to multi-day events based on their pre-computed week-level lanes
      allDayTasks
        .filter(item => item.seg)
        .sort((a, b) => a.seg.lane - b.seg.lane)
        .forEach(item => {
          decoratedDayTasks.push({ ...item, assignedLane: item.seg.lane });
        });
      
      // Second pass: assign lanes to single-day events starting AFTER all multi-day lanes
      // so they always appear below multi-day bars regardless of which lanes are occupied today
      let nextSingleDayLane = weekLayout.maxLanes;
      allDayTasks
        .filter(item => !item.seg)
        .sort((a, b) => a.originalIdx - b.originalIdx)
        .forEach(item => {
          decoratedDayTasks.push({ ...item, assignedLane: nextSingleDayLane });
          nextSingleDayLane++;
        });
      
      // Sort by assigned lane for consistent rendering order
      decoratedDayTasks.sort((a, b) => a.assignedLane - b.assignedLane);
            ...
            >
              ...
              {decoratedDayTasks.length > 0 && (
                <div className="calendar-day-tasks">
                  {decoratedDayTasks.slice(0, maxVisiblePerCell).map(({ t, seg, assignedLane }, renderIdx) => {
              ...
              {days.map((d, dayIdx) => {
                ...
                return (
                  <div key={`mwad-${d.toISOString()}`} className="mobile-week-allday-col">
                    {daySegments.map((seg) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={(e) => { e.stopPropagation(); openCalendarEntry(t); }}
                        >
                          <span>{t.title}{isHolidayEntry(t) && ' (Feiertag)'}</span>
                        </div>
                      );
                    })}
                    {regularTasks.map((t) => {
                    ...
                    {totalTasks.length > 2 && (
                      <div className="mobile-week-allday-more">+{totalTasks.length - 2}</div>
                    )}
                  </div>
                );
              })}
