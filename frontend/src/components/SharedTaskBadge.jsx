import { Lock, Users, UserCheck, Eye, Edit3 } from 'lucide-react';
import AvatarBadge from './AvatarBadge';

export default function SharedTaskBadge({ task }) {
  if (!task) return null;

  const { visibility, is_owner, creator_name, creator_color, can_edit, last_editor_name, shared_with_users, can_see_private_share_info } = task;
  const showReadOnlyBadge = is_owner === false && can_edit === false;
  const showEditableBadge = is_owner === false && can_edit === true;
  const canSeePrivateShareInfo = visibility !== 'selected_users' || can_see_private_share_info === true;

  if (visibility === 'private' || !visibility) return null;
  if (visibility === 'selected_users' && !canSeePrivateShareInfo) return showReadOnlyBadge ? (
  ...
  const sharedUsers = canSeePrivateShareInfo && Array.isArray(shared_with_users) ? shared_with_users : [];
  ...
  return (
    <div className="shared-task-badge">
      ...
      {showEditableBadge && (
        <span className="readonly-badge" style={{ background: 'rgba(52,199,89,0.12)', color: '#1f7a38', borderColor: 'rgba(52,199,89,0.35)' }}>
