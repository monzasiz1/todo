...
module.exports = async function handler(req, res) {
  ...
  if (action === 'resend-code' && req.method === 'POST') {
    try {
      ...
      try {
        const sendVerificationCodeMail = getSendVerificationCodeMail();
        if (sendVerificationCodeMail) {
          await sendVerificationCodeMail({ to: user.email, name: user.name, code: verificationCode });
        }
      } catch (mailErr) {
        console.error('Resend-Code Mail Fehler:', mailErr.message);
        return res.status(500).json({ error: 'E-Mail konnte nicht gesendet werden. Bitte später erneut versuchen.' });
      }

      return res.status(200).json({ success: true, message: 'Neuer Verifizierungscode wurde gesendet.' });
    } catch (err) {
